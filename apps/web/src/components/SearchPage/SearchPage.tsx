...
import { searchManga, type SearchResult, type SeriesResult, type VolumeResult } from '../../api/manga';
...
export function SearchPage({ initialQuery, onSelectSeries, onSelectBook, onSearch }: SearchPageProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    if (initialQuery && initialQuery !== lastSearchedQuery.current) {
      ...
      // Clear results when navigating back to empty search
  ...
  const handleSearch = useCallback((searchQuery: string) => {
    onSearch(searchQuery); // Update URL
    executeSearch(searchQuery, debugMode);
  }, [onSearch, executeSearch, debugMode]);
        ...
        <div className={styles.librarySelector}>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (results?.query) {
                executeSearch(results.query, debugMode);
              }
            }}
          >
            {libraries.map((lib) => (
            ...
            <section className={styles.bestMatchSection}>
              ...
              {results.bestMatch.type === 'series' && results.bestMatch.series && (
                <SeriesCard
                  ...
                  onClick={() => onSelectSeries(results.bestMatch!.series!.slug)}
                  highlighted
                />
              )}
